 	   ...
 	   for cnt=0 to (UBound(TEMP))
          if Not isNULL(TEMP) and TEMP(CNT) <> 0 then  
             ...
             IF  opt ="Y" THEN
                 ...
                 subSQL = "exec sp_atz_user_menu_update " & TEMP(CNT) &",'"& REQUEST("ReID") &"'  "
             '    response.Write subsql
             '    response.end 
                 Syscon.execute(subSQL)
    ...
    <TABLE style="margin:0;width:100%;" border=0 bgcolor="#D8E4F3">
      <Tr><form name=form action="sys_duty_left.asp" method="post">
		  <td>	
    ...
    function duty_save() {
        ...
        location.href='sys_duty_left.asp?opt='+opt+'&reid=<%=reid%>&slt=ADD&chk_pgid='+ param ;
    }
    </SCRIPT>    
